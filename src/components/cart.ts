...
class Cart {
    ...
    getItemNumbers(): number {
        let summ = 0;
        if (this.userCartArr && this.userCartArr[0]) {
            for (let i = 0; i < this.userCartArr.length; i++) {
                summ += this.userCartArr[i].quntity;
            }
        }
        return summ;
    }
    ...
    drawCart(userCart?: IUserCart): void {
        if (userCart) {
            ...
            const alreadyItem = this.userCartArr.some((v, i, a) => {
    ...
    drawTable(): void {
        const userS: string = localStorage.getItem('shopUser');
        if (!this.userCartArr[0]) this.userCartArr = JSON.parse(localStorage.getItem(userS));
        const cartVlaue: HTMLElement = document.querySelector('.cart-value') as HTMLElement | null;
        cartVlaue.innerText = this.getItemNumbers().toString();
        ...
        if (this.userCartArr && this.userCartArr[0]) {
            this.userCartArr.forEach((userCartItem: IUserCart) => {
                ...
                itemImg.style.backgroundImage = `url(${userCartItem.img || 'img/placeholder.jpg'})`;
